...
from alita_mini.crawler.app.nianbao_sft.extractor_pdf import TrainingDataExteactor


class Craw:
    def __init__(self, url_meta_file, sft_data_file):
        today = datetime.now()
        ...
        self.sft_data_file = sft_data_file
    ...
    def get_already_download_info(self):
        if os.path.exists(self.sft_data_file) is False:
            return set()
        reader = open(self.sft_data_file, "r")
        url_set = set()
        #  new_data = {"url": 'xxx', "conversation_id": f"{index}", "dataset": "school_math", "conversation": [ {"human": input, "assistant": output}]}
        while True:
            ...
            try:
                ...
                continue
            url_set.add(data_json.get("url", "none"))
    ...
    def download_meta_file(self):
        file = open(self.url_meta_file, "w")
    ...
    def craw(self, count=5):
        # 1.  抓取url, 年报， 半年报
        # self.download_meta_file()
        # 2. 从mongodb中获得已经下载的url
        already_download_set = self.get_already_download_info()
        # 3. 去掉重复url
        lines = open(self.url_meta_file).readlines()
        ...
        for item in data_list:
            ...
            if url in already_download_set:
                already_count += 1
                continue
            """
        ...
        file = open(self.sft_data_file, "w")
        # 4.下载抽取
    ...
    def download_and_process_file(self, url, local_path="/tmp/a.PDF", try_count=3):
        ...
        chunks = train_data_extractor.get_all_train_lines(local_path=local_path)
    ...
    def process_item(self, item, file):
        ...
        chunks = self.download_and_process_file(url, self.tmp_file)
        ...
        for index, chunk in enumerate(chunks):
            input = f"这是{stock_name}({stock_code}){pub_time}年发布的年报中的一个片段，请记住这个片段，未来你可以从这个内容中回答相关的问题。"
            new_data = {
                "url": url,
                "conversation_id": f"{index}",
                "dataset": "a_stock_report",
                "conversation": [{"human": input, "assistant": f"{title}中的第{index+1}个片段: {chunk}"}],
            }
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-m",
        "--meta_file",
        default="/Users/alchemy_taotaox/data/alita/nianbao_url_info.txt",
        help="command: [load|dump]",
    )
    parser.add_argument(
        "-d",
        "--data_file",
        default="/Users/alchemy_taotaox/data/alita/nianbao_sft_data.txt",
        help="command: [load|dump]",
    )

    args = parser.parse_args()

    craw = Craw(url_meta_file=args.meta_file, sft_data_file=args.data_file)
    craw.craw()
